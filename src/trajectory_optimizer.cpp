                           ...
                           std::vector<box> &boxes, States &result, pair<PointBoundry, PointBoundry> &tpb)
{
    ...
    while (iter < config_.opti_iter_max) {
      ...
      infea.push_back(cur_infeasibility);
    ...
    for (size_t i = 0; i < infea.size(); i++)
    {
      cout << "iter: " << i << "      infeasibility:  " << infea[i] << endl;
    }
...
ObsInfo TrajectoryOptimizer::getObsInfo(States &state, KDTree &obsTree)
{
    ObsInfo tmp;
    tmp.mid_nearest.resize(state.x.size());
    tmp.front_nearest.resize(state.xf.size());
    ...
    for (size_t i = 0; i < tmp.mid_nearest.size(); i++) {
        ...
        pt.push_back(state.x[i]);
        pt.push_back(state.y[i]);
        auto res_mid = obsTree.neighborhood_points(pt, 4);
        //tmp.mid_nearest[i] = std::make_pair(res_mid[0], res_mid[1]);
