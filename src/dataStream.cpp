...
void BoxDataToTxt(std::string txtPath, std::vector<std::vector<std::pair<double, double>>> &boxes)
{
    ...
    for(size_t i = 0; i < boxes.size(); i++){
        file << setprecision(10) << boxes[i][0].first  << ","
             << setprecision(10) << boxes[i][0].second << ","
             << setprecision(10) << boxes[i][1].first  << ","
             << setprecision(10) << boxes[i][1].second << "," 
             << setprecision(10) << boxes[i][2].first  << ","
             << setprecision(10) << boxes[i][2].second << "," 
             << setprecision(10) << boxes[i][3].first  << ","
             << setprecision(10) << boxes[i][3].second << std::endl;
    }
}
...
{
    std::ofstream file(txtPath, std::ios::out);
    for(size_t i = 0; i < refdis.size(); i++){
        file << setprecision(10) << refdis[i]  << ","
             << setprecision(10) << optdis[i] << std::endl;
    }
}
