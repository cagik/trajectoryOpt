...
void TrajectoryNLP::BuildIterativeNLP()
{
    ...
    //hard constraint
    auto t_g = cos(M_PI_2/ - p_ref_theta) * (x - p_ref_x) - sin(M_PI_2/ - p_ref_theta) * (y - p_ref_y);
    auto l_g = sin(M_PI_2/ - p_ref_theta) * (x - p_ref_x) + cos(M_PI_2/ - p_ref_theta) * (y - p_ref_y);
    ...
    //sigmoid
    SX f_obj =
      5 * (sumsqr(x - p_ref_x) + sumsqr(y - p_ref_y)) + config_.opti_w_r_theta * sumsqr(theta - p_ref_theta) +
      config_.opti_w_u * (sumsqr(jerk) + config_.opti_w_rw * sumsqr(omega)) + p_inf_w * infeasibility
      + p_inf_w * 1e-0 * sumsqr(pow(1 + pow(E_, (pow((xf - p_obs_xf)/1, 2) + pow((yf - p_obs_yf)/1, 2)) - 4), -1))
      + p_inf_w * 1e-0 * sumsqr(pow(1 + pow(E_, (pow((xr - p_obs_xr)/1, 2) + pow((yr - p_obs_yr)/1, 2)) - 4), -1));

    // SX f_obj =
    //    sumsqr(x - p_ref_x) + sumsqr(y - p_ref_y) + config_.opti_w_r_theta * sumsqr(theta - p_ref_theta) + 
    //    config_.opti_w_u * (sumsqr(jerk) + config_.opti_w_rw * sumsqr(omega)) + p_inf_w * infeasibility +
    //    p_inf_w * 1e0 * sumsqr(pow(E_ , 4 - (pow((x - p_obs_x)/1, 2) + pow((y - p_obs_y)/1, 2)))) +
    //    p_inf_w * 1e0 * sumsqr(pow(E_ , 4 - (pow((xf - p_obs_xf)/1, 2) + pow((yf - p_obs_yf)/1, 2)))) +
    //    p_inf_w * 1e0 * sumsqr(pow(E_ , 4 - (pow((xr - p_obs_xr)/1, 2) + pow((yr - p_obs_yr)/1, 2))));

    // SX f_obj =
    //  sumsqr(x - p_ref_x) + sumsqr(y - p_ref_y) + config_.opti_w_r_theta * sumsqr(theta - p_ref_theta) + 
    //  config_.opti_w_u * (sumsqr(jerk) + config_.opti_w_rw * sumsqr(omega)) + p_inf_w * infeasibility +
    //  p_inf_w * 1e-2 * sumsqr(1e-4 * pow((pow((x - p_obs_x)/1, 2) + pow((y - p_obs_y)/1, 2)), -2)) +
    //  p_inf_w * 1e-2 * sumsqr(1e-4 * pow((pow((xf - p_obs_xf)/1, 2) + pow((yf - p_obs_yf)/1, 2)), -2)) +
    //  p_inf_w * 1e-2 * sumsqr(1e-4 * pow((pow((xr - p_obs_xr)/1, 2) + pow((yr - p_obs_yr)/1, 2)), -2));

    // SX f_obj =
    //  sumsqr(x - p_ref_x) + sumsqr(y - p_ref_y) + config_.opti_w_r_theta * sumsqr(theta - p_ref_theta) + 
    //  config_.opti_w_u * (sumsqr(jerk) + config_.opti_w_rw * sumsqr(omega)) + p_inf_w * infeasibility +
    //  p_inf_w * 1e-1 * sumsqr(1e-2 * pow((pow((xf - p_obs_xf)/1, 2) + pow((yf - p_obs_yf)/1, 2)), -1)) +
    //  p_inf_w * 1e-1 * sumsqr(1e-2 * pow((pow((xr - p_obs_xr)/1, 2) + pow((yr - p_obs_yr)/1, 2)), -1));    
...
double TrajectoryNLP::SolveIteratively(double w_inf, const States &guess, const Trajectory &reference, ObsInfo &obs,
                          std::vector<box> &boxes, States &result, pair<PointBoundry, PointBoundry> &tpb)
{
    ...
    lb_x(0) = ub_x(0) = tpb.first.x;;
    lb_y(0) = ub_y(0) = tpb.first.y;
    lb_theta(0) = ub_theta(0) = tpb.first.theta;
    lb_v(0) = ub_v(0) = 5;
    lb_phi(0) = ub_phi(0) = 0;
    lb_a(0) = ub_a(0) = 0;
    lb_omega(0) = ub_omega(0) = 0;
    ...
    {
        lb_gl(i) = boxes[i].l_lb;
        ub_gl(i) = boxes[i].l_ub;
        lb_gt(i) = boxes[i].t_lb;
        ub_gt(i) = boxes[i].t_ub;
    }
    ...
    arg["lbg"] = DM::vertcat({lb_gt, lb_gl});
    arg["ubg"] = DM::vertcat({ub_gt, ub_gl});
    arg["x0"] = DM::vertcat(
