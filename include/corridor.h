...
public:
    ...
    //得到安全走廊的外部接口
    void update(const vector<pair<double, double>> traj, double margin, bool updateMode);
...
private:
    ...
    bool isPointInObstacle(double x, double y, double margin);

    //延展box
    void expandBox(vector<double>& box, double margin);

    void changeRef(vector<double>& box, double margin);

    //产生box的私有函数
    void updateObsBox(double margin, bool updateMode);
